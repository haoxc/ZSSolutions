// Line-by-line replacement of ASCII fenced blocks with Mermaid/Vega
const fs = require('fs');
const file = process.argv[2];
let content = fs.readFileSync(file, 'utf-8');
let lines = content.split('\n');

// Map: first_line_substring → [replacement_lines_array, "mermaid"|"vega-lite"]
const map = new Map();
...
// fig1-1: 方案核心逻辑
add('方案核心逻辑：从三座矛盾山到 USDF 一体化', 'mermaid',
'flowchart TB',
'    subgraph 三大结构性痛点',
'    M1["痛点1<br/>设备多样 vs 管理统一<br/>百万测点·协议不兼容"]',
'    M2["痛点2<br/>实时控制 vs 数据洪流<br/>端到端≤1s·暂态≤100ms"]',
'    M3["痛点3<br/>智能预期 vs 数字基座<br/>AI即插即用·标准化对象"]',
'    end',
'    M1 --> USDF',
'    M2 --> USDF',
'    M3 --> USDF',
'    subgraph USDF["USDF 统一空间数据底座"]',
'    direction LR',
'    P1["P1 暂态通道 ≤100ms"]',
'    P2["P2 稳态通道 ≤1s"]',
'    P3["P3 历史通道 ≤5s"]',
'    API["北向 API · RESTful"]',
'    end',
'    USDF --> R1["✅ 可靠<br/>暂态独立通道"]',
'    USDF --> R2["✅ 扩展<br/>station_id 预留"]',
'    USDF --> R3["✅ AI友好<br/>算法不碰采集层"]',
'    style USDF fill:#1f4e79,color:#fff',
'    style M1 fill:#c00000,color:#fff',
'    style M2 fill:#c00000,color:#fff',
'    style M3 fill:#c00000,color:#fff',
...
// 逻辑四层
add('展示层    三维 Web 门户 / 大屏 / 移动端 / 运维门户', 'mermaid',
'flowchart TB',
'    subgraph L4["展示层"]',
...
// 四层运维监控体系 - keep as ASCII (V2 presents this as a table)

// 系统完整架构
add('系统完整架构（从采集到展示）', 'mermaid',
'flowchart TB',
...
// 告警工单闭环时序
add('告警工单闭环时序（AIGen/工单中心/运维人员/审核人 交互序列）', 'mermaid',
'sequenceDiagram',
...
// fig5-1: 四层 SLA
add('四层 SLA 延迟预算与数据通路', 'vega-lite',
'{',
...
// fig6-1: 甘特图
add('项目实施甘特图（4 个月，三阶段交付）', 'mermaid',
'gantt',
...
// fig7-1: 风险热力图
add('风险热力图（18 项风险 x 三维度）', 'vega-lite',
'{',
'  "$schema": "https://vega.github.io/schema/vega-lite/v6.json",',
...
while (i < lines.length) {
  ...
  // Check if this is the start of a plain ``` block (not ```mermaid or ```vega-lite)
  if (line.trim() === '```' && (i + 1 < lines.length) &&
      !lines[i + 1].trim().startsWith('```') &&  // skip ```
    ...
    const blockEnd = i; // closing ```

    // Check first content line against map
    const firstLine = (blockContentLines[0] || '').trim();
    let matched = false;

    for (const [key, replacement] of map) {
    ...
    if (!matched) {
      // Keep original block
      newLines.push(lines[blockStart]); // opening ```
      newLines.push(...blockContentLines);
      newLines.push(lines[blockEnd]); // closing ```
      console.log(`Kept ASCII: ${firstLine.substring(0, 60)}`);
    }
